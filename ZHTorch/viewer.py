...
class HabitatSimInteractiveViewer(Application):
    ...
    def reconfigure_sim(self) -> None:
        ...
        # instance_id_to_label_id = {int(obj.id.split("_")[-1]): obj.category.index() for obj in scene.objects if obj is not None}
        label_id2colorid = {k:i for i, k in enumerate(sorted(set(instance_id_to_label_id.values())))}
        mapping = np.ones((500,)) * 39
        for i in range(np.max(list(instance_id_to_label_id.keys()))+1):
            if i not in instance_id_to_label_id.keys():
                continue
            else:
                colorid = label_id2colorid[instance_id_to_label_id[i]]
                if colorid>=0 and colorid < 40:
                    mapping[i] = colorid
    ...
    def save_semantic_observation(self, obs, total_frames):
        ...
        label_obs = np.take(self.mapping, semantic_obs)
        semantic_img = Image.new("P", (semantic_obs.shape[1], semantic_obs.shape[0]))
        semantic_img.putpalette(d3_40_colors_rgb.flatten())
        semantic_img.putdata((label_obs.flatten()).astype(np.uint8))
